...
function returnInvoiceList($id){
  ...
  $invoices = $db->query("SELECT id_invoice AS id, reference, invoice_date, people.firstname AS firstname, people.lastname AS lastname FROM invoice INNER JOIN people ON invoice.fk_people = people.id_people WHERE invoice.fk_company = $id ORDER BY id_invoice DESC");
